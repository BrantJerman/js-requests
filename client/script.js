...
function setHelloButtonBlack ()
{
    sayHelloButton.style.background = 'black';
}

sayHelloButton.addEventListener('mouseover', setHelloButtonBlack);
...
function setHelloBtnToNormal () {
    sayHelloButton.style.background = '#EFEFEF ';
}

sayHelloButton.addEventListener('mouseout', setHelloBtnToNormal)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/Bears`
    ) .then(function (response) {
        ...
        // console.log(document.getElementById('repeat-text').textContent)
    })
}
...
const newFunction = () => {
    axios.get('http://localhost:3000/query-test/?query=brant')
        .then(function (response) {
            console.log(response.data)
        })
    }

document.getElementById('query-button').addEventListener('click', newFunction)
